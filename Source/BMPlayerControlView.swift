...
open class BMPlayerControlView: UIView {
    ...
    open @objc func progressSliderTouchBegan(_ sender: UISlider)  {
      delegate?.controlView(controlView: self, slider: sender, onSliderEvent: .touchDown)
    }
    
    open @objc func progressSliderValueChanged(_ sender: UISlider)  {
      hidePlayToTheEndView()
      cancelAutoFadeOutAnimation()
    ...
    open @objc func progressSliderTouchEnded(_ sender: UISlider)  {
      autoFadeOutControlViewWithAnimation()
